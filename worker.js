export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/edit-image' && request.method === 'POST') {
      try {
        ...
        if (submitData?.output?.image) {
          return await proxyImageResult(submitData.output.image);
        }
        ...
        if (!result) {
          return jsonResponse({ error: '任务超时' }, 500);
        }
        ...
        if (typeof imageCandidate === 'string' && imageCandidate.startsWith('http')) {
          return await proxyImageResult(imageCandidate);
        }
        ...
        return jsonResponse({ error: error?.message || 'Worker 处理失败' }, 500);
      }
    }
